...
func SearchNode(req model.SearchReq, useFullText bool) ([]model.SearchNode, int64, error) {
	...
	if !useFullText || conf.Conf.Database.Type == "sqlite3" {
		...
		for _, keyword := range strings.Fields(req.Keywords) {
			keywordsClause = keywordsClause.Or("description LIKE ?", fmt.Sprintf("%%%s%%", keyword)).
                                Or("name LIKE ?", fmt.Sprintf("%%%s%%", keyword))
		}
		...
		case "mysql":
			searchDB = db.Model(&model.SearchNode{}).Where(whereInParent(req.Parent)).
				Where("MATCH (ndescription) AGAINST (? IN BOOLEAN MODE)", "'*"+req.Keywords+"*'")
		case "postgres":
			searchDB = db.Model(&model.SearchNode{}).Where(whereInParent(req.Parent)).
				Where("to_tsvector(description) @@ to_tsquery(?)", strings.Join(strings.Fields(req.Keywords), " & "))
		}
	}
