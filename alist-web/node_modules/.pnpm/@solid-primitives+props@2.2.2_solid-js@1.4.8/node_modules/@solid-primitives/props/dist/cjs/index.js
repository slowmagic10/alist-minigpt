...
const SelectProp = props => {
  const options = solidJs.createMemo(() => Array.isArray(props.options) ? props.options.map(option => [option, option]) : filterEnum(props.options));
  ...
  return (() => {
    ...
    web.insert(_el$16, web.createComponent(solidJs.For, {
      ...
      children: ([key], index) => (() => {
        ...
        web.insert(_el$18, key);

        web.effect(() => _el$18.value = index());
  ...
  return new Proxy(merge, {
    get(target, key) {
      ...
      if (isEventListenerKey(key)) {
        ...
        return Array.isArray(callbacks) ? utils.chain(callbacks) : Reflect.get(target, key);
...
exports.createControlledProp = createControlledProp;
exports.createControlledProps = createControlledProps;
